...
if (cross_check_fingerprints) {
  ...
  if (!scatter_cross_check_fingerprints) {
    ...
    call Tasks.CrossCheckFingerprint as CrossCheckFingerprintSolo {
      input:
        ...
        vcf_paths = HardFilterAndMakeSitesOnlyVcf.sites_only_vcf,
        sample_name_map = sample_name_map,
        haplotype_database = haplotype_database,
  ...
  File output_detail_metrics_file = select_first([CollectMetricsOnFullVcf.detail_metrics_file])
  File output_summary_metrics_file = select_first([CollectMetricsOnFullVcf.summary_metrics_file])

  # Get the VCFs from either code path
  Array[File?] output_vcf_files = [FinalGatherVcf.output_vcf] #else SitesOnlyGatherVcf.output_vcf
  Array[File?] output_vcf_index_files = [FinalGatherVcf.output_vcf_index] #else ApplyRecalibration.recalibrated_vcf_index
